...
#     get_updated_covid_data(address)
    ...
    for i in range(1,r):
        ...
        temp_data = pd.read_csv(previous_r_root + 'validation/tables/tabel_of_best_validation_results.csv')
        previous_r_val_result = temp_data.loc[(temp_data['method']=='KNN')| (temp_data['method']=='GBM') | (temp_data['method']=='NN')]
        ...
        print(previous_r_root + 'test/all_errors/'+str(best_method)+'/all_errors_'+str(best_method)+'.csv')
        previous_r_test_result = pd.read_csv(previous_r_root + 'test/all_errors/'+str(best_method)+'/all_errors_'+str(best_method)+'.csv')
        result.iloc[-(i+1),result.columns.tolist().index('Target')] = previous_r_test_result['prediction'].iloc[0]
