...
#         else:
        ...
        X_train_val = main_data.iloc[:-((test_size + base_r-1) * numberOfCounties)].tail(val_size * numberOfCounties).copy()
        X_train_train = main_data.iloc[:-((val_size + test_size + base_r-1) * numberOfCounties)].copy()

        y_test = target.tail(test_size * numberOfCounties).copy()
        y_train_val = target.iloc[:-((test_size + base_r-1) * numberOfCounties)].tail(val_size * numberOfCounties).copy()
        y_train_train = target.iloc[:-((val_size + test_size + base_r-1) * numberOfCounties)].copy()
...
#         else:
        X_test = main_data.tail(test_size * numberOfCounties).copy()
        X_train = main_data.iloc[:-((test_size + base_r-1) * numberOfCounties)].copy()

        y_test = target.tail(test_size * numberOfCounties).copy()
        y_train = target.iloc[:-((test_size + base_r-1) * numberOfCounties)]
