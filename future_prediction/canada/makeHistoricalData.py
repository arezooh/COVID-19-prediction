...
def makeHistoricalData(h, r, test_size, target, feature_selection, spatial_mode, target_mode,
    ...
    ranking_data = ranking_data.iloc[:-(end_date+r),:]
    ...
    if os.path.exists("./canada/"+target_mode+"/"+str(r)+"/") and end_date == 0 :
        pd.DataFrame(np.array(ix),columns=['name']).to_csv("./canada/"+target_mode+"/"+str(r)+"/covariates_rank.csv",index=False)
...
def main():
    ...
    future_features = []
    test_size = 21
    result = makeHistoricalData(0, r, test_size, target, 'mrmr', spatial_mode, target_mode, address,
