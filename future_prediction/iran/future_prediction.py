...
def create_tables(y_prediction, y_test_date, y_train_date, r, country_errors, temporal_mode, daily_output,
    ...
    if temporal_mode == 'daily':
      ...
      y_train_date['Date'] = y_train_date['date of day t'].apply(
                  lambda x: datetime.datetime.strptime(x, '%m/%d/%y') + datetime.timedelta(days=r*7))
      y_test_date['Date'] = y_test_date['date of day t'].apply(
                  lambda x: datetime.datetime.strptime(x, '%m/%d/%y') + datetime.timedelta(days=r*7))
...
def main():
    ...
    temporal_data = generate_updated_temporal_data(data_address)
    ...
    for r in range(1,10+1):
        ...
        test_point_limit = test_point_limit_r_1 - (2*(r-1))
        
        for test_point in range(test_point_limit+1):
            print(100 * '#')
            print('test_poitnt = ',test_point)
##            subprocess.call("python ./prediction.py "+str(r)+" weeklyaverage "+str(test_point), shell=True)
...
#         regular_data.to_csv(str(h)+'-'+str(r)+'regular_data.csv',index=False)
        ...
        for point in range(1,r+1):
            ...
            if temporal_mode == 'weekly':
                ...
                if future_point == r-1: #first future point
                    # add training part
                    Final_y_prediction = list(y_prediction[:-(1+test_point_limit+1)])
                    # add previous test points
                    for test_point in range(test_point_limit+1):
                        test_index = test_point_limit - test_point
                        test_point_value = get_test_point_value(address,test_index,r)
                        Final_y_prediction.append(test_point_value)

                    # add future prdiction
                    Final_y_prediction = Final_y_prediction + list(y_prediction[-1:])

                    y_prediction = np.array(Final_y_prediction)
